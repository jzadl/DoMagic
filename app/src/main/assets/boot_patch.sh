...
#   magiskinit, magisk, magiskboot, stub.apk, init-ld.so, util_functions.sh
#
#######################################################################################
...
export KEEPVERITY KEEPFORCEENCRYPT PATCHVBMETAFLAG
...
case $? in
  ...
  3 ) ui_print "- Vendor boot image detected"; VENDORBOOT=true ;;
...
./magiskboot compress=xz init-ld.so init-ld.so.xz
...
for dt in dtb kernel_dtb extra; do
  if [ -f $dt ]; then
    ...
    ./magiskboot dtb $dt patch && ui_print "- Patch fstab in boot image $dt"
  fi
done
...
if [ -f kernel ]; then
  ...
  ./magiskboot hexpatch kernel \
    ...
    && PATCHEDKERNEL=true
  # Force kernel to load rootfs for legacy SAR
